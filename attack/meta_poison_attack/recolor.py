...
def recolor(inputs, colorperts, name=None, grid=None):
    ...
    def trilinear_interpolation(imgs, colorgrid, xrefmin, xrefmax):
        ...
        inter_idx = tf.stack([tf.cast((imgs[..., i] - xrefmin[i]) * gridshape[i], dtype=dtype) for i in range(len(xrefmin))], 
                             axis=-1)
        ...
        for y_i in [0, 1]:
            for u_i in [0, 1]:
                for v_i in [0, 1]:
                    ...
                    """
                    idx = tf.stack([tf.stack(inter_idx[...,i]+x) for i, x in enumerate([y_i, u_i, v_i])],
                                   axis=-1)
